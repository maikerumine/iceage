...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if above.name =="air" and minetest.get_node_light(p)>LIGHT_MAX*0.7 then -- snow melts -- minetest.get_node_light(p)>LIGHT_MAX-7
			minetest.set_node(pos, {name="iceage:snowblock"})
		end
	end,
})
...
minetest.register_abm({
	...
	action = function(pos, node)
		...
		if name == "default:dirt_with_snow" or name == "default:dirt_with_grass" or name == "iceage:snowblock"  then
			pos.y = pos.y+1
			local height = 0
			while minetest.get_node(pos).name == "iceage:snowblock" and height < 1 do
				height = height+1
				pos.y = pos.y+1
			end
			if height < 1 then
