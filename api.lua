iceage = {}
iceage.mod = "redo-redo"
function iceage:register_mob(name, def)
...
--[[				self.object:setacceleration({x=x, y=-10, z=z})
 ...
 --Mobs float in water now, to revert uncomment previous 4 lines and remove following block of 12
			...
			-- ICE FIND A BADPLAYER TO ATTACK
			if self.type == "npc" and self.attacks_monsters and self.state ~= "attack" then
				...
				for _, oir in pairs(inradius) do
					local obj = oir:get_luaentity()
					if obj then
						if obj.type == "badp" or obj.type == "monster" then
							-- attack monster
...
iceage.spawning_mobs = {}
function iceage:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, min_dist, max_dist, spawn_func)
	iceage.spawning_mobs[name] = true
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			if not iceage.spawning_mobs[name] then
				return
			end
